...
async def migrate_database():
    ...
    try:
        ...
        for c in cookie:
            ...
            logger.info('脑积水数据库迁移', f'成功迁移公共cookie<m>{c[0][:20]}...</m>')
    except Exception:
        logger.info('脑积水数据库迁移', '公共cookie没有可迁移的数据')
    # 迁移私人cookie
    try:
        ...
        for c in cookie:
            ...
            logger.info('脑积水数据库迁移', f'成功迁移用户<m>{c[0]}</m>的UID<m>{c[1]}</m>的<m>私人cookie</m>')
    except Exception:
        logger.info('脑积水数据库迁移', '私人cookie没有可迁移的数据')
    # 最后查询记录迁移
    try:
        ...
        logger.info('脑积水数据库迁移', f'成功迁移UID查询记录<m>{len(query)}</m>条')
    except Exception:
        logger.info('脑积水数据库迁移', 'UID查询记录没有可迁移的数据')
    # 实时便签提醒迁移
    try:
        ...
        for n in note:
            ...
            logger.info('脑积水数据库迁移', f'成功迁移用户<m>{n[0]}</m>的UID<m>{n[1]}</m>的米游社自动签到')
    except Exception:
        logger.info('脑积水数据库迁移', '米游社自动签到没有可迁移的数据')
    # 米游社签到迁移
    try:
        ...
        for s in sign:
            ...
            logger.info('脑积水数据库迁移', f'成功迁移用户<m>{s[0]}</m>的UID<m>{s[1]}</m>的米游社原神签到')
    except Exception:
        logger.info('脑积水数据库迁移', '米游社原神签到没有可迁移的数据')
    # 米游币获取迁移
    try:
        ...
        for s in sign:
            ...
            logger.info('脑积水数据库迁移', f'成功迁移用户<m>{s[0]}</m>的UID<m>{s[1]}</m>的米游币自动获取')
    except Exception:
        logger.info('脑积水数据库迁移', '米游币自动获取没有可迁移的数据')
